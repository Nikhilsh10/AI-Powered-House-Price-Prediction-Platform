# src/training/train_xgboost.py
"""Training script for XGBoost model.
It loads the cleaned dataset, performs preprocessing, trains the model,
computes evaluation metrics, and saves the model and preprocessor artifacts.
"""

import json
import joblib
import pandas as pd
import numpy as np
from sklearn.model_selection import train_test_split
from sklearn.compose import ColumnTransformer
from sklearn.preprocessing import OneHotEncoder, StandardScaler
from sklearn.metrics import r2_score, mean_absolute_error, mean_squared_error
from xgboost import XGBRegressor

from src.training.config import (
    CLEAN_DATA_PATH,
    FEATURE_METADATA_PATH,
    XGB_MODEL_PATH,
    PREPROCESSOR_PATH,
    METRICS_PATH,
    XGB_PARAMS,
)

def load_data():
    df = pd.read_csv(CLEAN_DATA_PATH)
    return df

def load_feature_metadata():
    with open(FEATURE_METADATA_PATH, "r") as f:
        meta = json.load(f)
    return meta

def get_target_column(df: pd.DataFrame) -> str:
    # The dataset uses 'price' as the target for Bengaluru data
    for col in ["price", "Price", "SalePrice"]:
        if col in df.columns:
            return col
    raise KeyError("Target price column not found in dataset")

def preprocess(df: pd.DataFrame, target_col: str):
    X = df.drop(columns=[target_col])
    y = df[target_col]
    # Identify column types
    categorical_cols = X.select_dtypes(include=["object", "category"]).columns.tolist()
    numeric_cols = X.select_dtypes(include=[np.number]).columns.tolist()
    # Build ColumnTransformer
    preprocessor = ColumnTransformer(
        transformers=[
            ("num", StandardScaler(), numeric_cols),
            ("cat", OneHotEncoder(handle_unknown="ignore", sparse_output=False), categorical_cols),
        ]
    )
    X_processed = preprocessor.fit_transform(X)
    return X_processed, y.values, preprocessor

def train_and_save():
    df = load_data()
    target_col = get_target_column(df)
    X_processed, y, preprocessor = preprocess(df, target_col)
    # Train‑test split
    X_train, X_test, y_train, y_test = train_test_split(
        X_processed, y, test_size=0.2, random_state=42
    )
    # Model
    model = XGBRegressor(**XGB_PARAMS)
    model.fit(X_train, y_train)
    # Save artifacts
    joblib.dump(model, XGB_MODEL_PATH)
    joblib.dump(preprocessor, PREPROCESSOR_PATH)
    # Evaluation
    preds = model.predict(X_test)
    r2 = r2_score(y_test, preds)
    mae = mean_absolute_error(y_test, preds)
    rmse = mean_squared_error(y_test, preds, squared=False)
    # MAPE (avoid division by zero)
    mape = np.mean(np.abs((y_test - preds) / np.where(y_test == 0, 1e-8, y_test)))*100
    metrics = {
        "r2": r2,
        "mae": mae,
        "rmse": rmse,
        "mape": mape,
    }
    # Write model‑specific metrics file for later aggregation
    model_metrics_path = os.path.join(os.path.dirname(XGB_MODEL_PATH), "xgboost_metrics.json")
    with open(model_metrics_path, "w") as f:
        json.dump(metrics, f, indent=2)
    print("XGBoost training complete. Metrics:", metrics)
    return metrics

if __name__ == "__main__":
    train_and_save()
